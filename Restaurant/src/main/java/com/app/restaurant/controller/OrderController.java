...
public class OrderController {
    ...
    public String paymentGET(HttpSession session, Model model) {
        if (session.getAttribute("order") != null) {
            ...
            model.addAttribute("price", orderService.calculatePriceForOrder(order));
    ...
    public String moveToDotpay(@RequestHeader String host, HttpSession session) {
        if (session.getAttribute("order") != null) {
            ...
            if (prior == 1) {
                amount = orderService.calculatePriceForOrder(order).add(new BigDecimal(10));
            } else {
                amount = orderService.calculatePriceForOrder(order);
            }
